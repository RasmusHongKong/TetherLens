...
def nlg_attachment_claims(body: str) -> list[CandidateClaim]:
...
def test_incomplete_physical_interface_claims_do_not_become_connection_interface():
    claims = [CandidateClaim(subject_type=ClaimSubjectType.PHYSICAL_INTERFACE, subject_ref="ambiguous_ring", property_key="interface.type", value="ring", source_url="https://example.test/attachment", extractor="test")]
    assert resolve_connection_interfaces(claims) == []


def test_nlg_explicit_d_ring_tether_point_resolves_as_provided_interface():
    interfaces = resolve_connection_interfaces(nlg_attachment_claims("The D Ring creates a secure tether point to attach a tool lanyard."))
    assert len(interfaces) == 1
...
def test_nlg_inline_markup_preserves_d_ring_tether_point_evidence():
    claims = NLGAdapter().extract(ProductIdentity(manufacturer="NLG", product_type=ProductType.TOOL_ATTACHMENT, name="D Ring Attachment", sku="example", url="https://example.test/nlg/attachment"), [artifact("<p>The <strong>D Ring</strong> creates a secure tether point.</p>")])
...
def test_nlg_block_boundary_does_not_join_unrelated_ring_and_tether_point_copy():
    claims = NLGAdapter().extract(ProductIdentity(manufacturer="NLG", product_type=ProductType.TOOL_ATTACHMENT, name="D Ring Attachment", sku="example", url="https://example.test/nlg/attachment"), [artifact("<p>D Ring construction</p><p>Creates a secure tether point.</p>")])
    assert resolve_connection_interfaces(claims) == []


def test_nlg_pre_ring_negation_blocks_prohibited_lanyard_relation():
    assert resolve_connection_interfaces(nlg_attachment_claims("Do not use the D Ring to attach a tool lanyard.")) == []


def test_nlg_pre_ring_permission_and_safety_prohibitions_block_interface_claims():
    prohibited = ("It is not permitted to use the D Ring to attach a tool lanyard.", "It is not allowed to use the D Ring to attach a tool lanyard.", "It is not safe to use the D Ring to attach a tool lanyard.")
...
def test_nlg_avoidance_prohibitions_block_interface_claims():
    prohibited = ("Avoid using the D Ring to attach a tool lanyard.", "Using the D Ring to attach a tool lanyard should be avoided.")
    for body in prohibited:
        assert resolve_connection_interfaces(nlg_attachment_claims(body)) == [], body


def test_nlg_positive_instrumental_avoidance_is_bounded_to_whole_assertion():
    interfaces = resolve_connection_interfaces(nlg_attachment_claims("Avoid snagging by using the D Ring to attach a tool lanyard."))
    assert len(interfaces) == 1
    assert interfaces[0].role == ConnectionInterfaceRole.TOOL_ATTACHMENT_TETHER_SIDE
    assert interfaces[0].interface_type == "ring"

    prohibited = (
        "Do not secure tools by using the D Ring to attach a tool lanyard.",
        "It is prohibited to secure tools by using the D Ring to attach a tool lanyard.",
    )
    for body in prohibited:
        assert resolve_connection_interfaces(nlg_attachment_claims(body)) == [], body
...
def test_nlg_unrelated_pre_ring_negation_does_not_suppress_positive_relation():
    interfaces = resolve_connection_interfaces(nlg_attachment_claims("This is not a disposable component; the D Ring creates a secure tether point."))
    assert len(interfaces) == 1
    assert interfaces[0].role == ConnectionInterfaceRole.TOOL_ATTACHMENT_TETHER_SIDE
    assert interfaces[0].interface_type == "ring"


def test_nlg_subject_switch_does_not_attribute_loop_lanyard_relation_to_d_ring():
    ambiguous = ("The D Ring is for ID tags, while the reinforced loop connects a tool lanyard.", "The D Ring is for ID tags and the reinforced loop connects a tool lanyard.")
...
def test_nlg_direct_negation_of_tether_relation_emits_no_interface():
    prohibited = ("The D Ring does not create a secure tether point.", "The D Ring does not connect to a tool lanyard.")
    for body in prohibited:
        assert resolve_connection_interfaces(nlg_attachment_claims(body)) == [], body


def test_nlg_positive_usage_and_reversed_tether_point_forms_are_supported():
    positive = ("Use the D Ring to attach a tool lanyard.", "Secure tether point with 360° rotating D-ring.")
    for body in positive:
        interfaces = resolve_connection_interfaces(nlg_attachment_claims(body))
...
def test_nlg_bare_d_ring_or_loop_evidence_does_not_invent_provided_interface():
    claims = NLGAdapter().extract(ProductIdentity(manufacturer="NLG", product_type=ProductType.TOOL_ATTACHMENT, name="D Ring Loop Tool Tether", sku="example", url="https://example.test/nlg/attachment"), [artifact("Climbing cord loop allows quick cinching. D Ring construction.")])
    assert resolve_connection_interfaces(claims) == []


def test_tether_endpoint_claims_resolve_to_runtime_connection_interfaces():
    claims = NLGAdapter().extract(ProductIdentity(manufacturer="NLG", product_type=ProductType.TETHER, name="Retractable Tool Lanyard", sku="example", url="https://example.test/nlg/tether"), [artifact("Integral carabiner connects to the belt anchor while the Rotobiner provides tool attachment. Max Load: 3 kg.")])
    interfaces = resolve_connection_interfaces(claims)
...
def test_tool_side_endpoint_to_attachment_ring_is_unresolved_without_geometry():
    endpoint = ConnectionInterface(interface_id="tool_endpoint", role=ConnectionInterfaceRole.TETHER_CONNECTION, interface_type="carabiner", tether_side=TetherSide.TOOL_SIDE, connector_spec_ref="rotobiner")
...
def test_anchor_side_only_endpoint_cannot_serve_tool_attachment_interface():
    endpoint = ConnectionInterface(interface_id="anchor_endpoint", role=ConnectionInterfaceRole.TETHER_CONNECTION, interface_type="carabiner", tether_side=TetherSide.ANCHOR_SIDE)
    target = resolve_connection_interfaces(provided_ring_claims())[0]
    result = evaluate_endpoint_engagement(endpoint, target)
...
def test_connection_evaluation_is_identity_agnostic_for_equivalent_topology():
    target = resolve_connection_interfaces(provided_ring_claims())[0]
    first = ConnectionInterface(interface_id="brand_a_endpoint", role=ConnectionInterfaceRole.TETHER_CONNECTION, interface_type="carabiner", tether_side=TetherSide.EITHER)
    second = ConnectionInterface(interface_id="brand_b_endpoint", role=ConnectionInterfaceRole.TETHER_CONNECTION, interface_type="carabiner", tether_side=TetherSide.EITHER)
...
def test_klein_nlg_attachment_tether_vertical_slice_stops_at_missing_engagement_geometry():
    tool_claims = KleinAdapter().extract(ProductIdentity(manufacturer="Klein Tools", product_type=ProductType.TOOL, name="Insulated Screwdriver", sku="6826INS", url="https://example.test/klein/6826ins"), [artifact("The tether hole in the handle provides added safety when working at height.")])
    features = resolve_tool_interface_features(tool_claims)
    attachment_claims = NLGAdapter().extract(ProductIdentity(manufacturer="NLG", product_type=ProductType.TOOL_ATTACHMENT, name="360 D Ring Loop Tool Tether", sku="101363", url="https://example.test/nlg/101363"), [artifact("The D Ring creates a secure tether point to attach a tool lanyard. Create a tether point on any tool with a captive hole or handle and cinch it around the tool.")])
    ...
    tether_claims = NLGAdapter().extract(ProductIdentity(manufacturer="NLG", product_type=ProductType.TETHER, name="Retractable Tool Lanyard", sku="example", url="https://example.test/nlg/tether"), [artifact("Integral carabiner connects to the belt anchor while the Rotobiner provides tool attachment. Max Load: 3 kg.")])
