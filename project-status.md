...
PR #57 keeps secure installed fit in the existing pre-use-obligation lane rather than adding a new hard-evaluation state. A structurally eligible ToolAttachment may therefore remain `recommended_with_constraints` while `secure_attachment_fit_required` is pending; a failed fit confirmation rejects the active candidate through the existing session semantics.
...
The two V2 C cases — GRIPPS H01150 SnapLock and 3M 1500028 Quick Spin Medium — remain frozen as C because the V2 artifact records the post-PR #55 architecture state. PR #57 resolves their **shared current-core handle subset** after inspecting first-party installation evidence: the reusable composition is a non-captive handle predicate plus a separate secure-installed-fit pre-use condition, not an inferred dimensional envelope. `non_captive_handle_attachment` contains only `feature_kind = handle` and `captive_state = non_captive`; `secure_attachment_fit_required` remains independent of `pre_use_attachment_test_required`.
...
PR #56 identified the recurring **non-captive ToolAttachment capture/fit** question. PR #57 resolves the common handle subset in the evolved current core: `non_captive_handle_attachment` provides a conservative handle-only compiler path with explicit `NON_CAPTIVE` state, while `secure_attachment_fit_required` captures manufacturer-required snug/secure installed fit as a distinct pre-use obligation. The runtime feature model did not need new anatomy fields and the existing candidate/session machinery remains authoritative for exact component/feature binding.
...
### 1. Vertically prove the non-captive handle / secure-fit boundary
...
- emit `non_captive_handle_attachment` only from evidence that actually authorizes installation on a non-captive handle;
...
- `non_captive_handle_attachment` is a narrow feature-state composition, not a dimensional-fit assertion; it must not gain size or diameter predicates without source-backed fit evidence;
